...
class Page(object):
	def __init__(self, url, query = None):
		...
		self.url = url
		self.raw_html = None
		self.crawl_date = elf.start_date = date.today()
		self.status = None
...
<<<<<<< HEAD
	...
	def status(self):
		...
		self.bad_status_msg = {}
		for n in ["url", "query", "error_code", "type", "status", "date", "crawl_date"]:
			print self.__dict__[n]
			self.bad_status_msg[n] = self.__dict__[n]
		return self.bad_status_msg
		# try:
