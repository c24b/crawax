...
class Database(object):
	...
	def __init__(self, database_name):
		...
		self._client = MongoClient('mongodb://localhost,localhost:27017')
		self.db = self._client[str(self.db_name)]
		#defaut collection
		self.collection = self._client['data']
		...
		#setting coll as attributes of db 
		#self.db.x = self.db[x]
	...
	def use_db(self, database_name):
		return self._client[str(database_name)]
	...
	def use_coll(self, coll_name):
		if self.db[str(coll_name)]:
			return self.db[str(coll_name)]
		else:
			return self.create_coll(coll_name)
	...
	def use_colls(self, coll_names=["results","sources", "log", "queue"]):
		for n in coll_names:
			setattr(self, n, self.db[str(n)])
		return self.db
	...
	def drop(self, type, name):
		if type == "collection":
			...
			return self._client.drop_database(str(name))
		else:
			print "Unknown Type"
			return False
	...
	def export(self, mode="csv"):
		'''Dump export of DB will be integrated to database'''
		# if mode == "json":
		# 	self.db.command('mongoexport', '-d', self.project, '-c', 'results', '--jsonArray', '-o', 'report.json')
		# elif mode == "csv":
		# 	self.db.command('mongoexport', '-d', self.project, '-c', 'results', '--csv', )
		#elif mode == "gephi":
			#pass
		#else:
			#defaut export
